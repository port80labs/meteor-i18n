...
i18n.get = function i18nGet(key, lang, ...vars) {
  ...
  return joinFormat(maybeGet(i18n.reactiveQuery({
    key: key
  }), lang), vars)
}

i18n.getAll = function i18nGetAll(key, ...vars) {
  return joinFormatObj(i18n.reactiveQuery({
    key: key
  }), vars)
}
...
i18n.listLanguages = function i18nListLanguages() {
  return _.map(i18n.state.get('langs'), (name, key) => {
    return {
      key,
      name
    }
...
if (Meteor.isServer) {
  i18n.add = function i18nAdd(data, lang, parent) {
    _.each(splitFormat(flattenObj(data, !lang)), (translation, key) => {
      var transObject = lang
        ? {
          [lang]: translation
        }
        : translation;
      for (var langKey in transObject) {
        if (i18n.db.find({
            key: key,
            [langKey]: {
              $exists: true,
